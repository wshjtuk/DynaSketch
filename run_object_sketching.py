...
parser.add_argument("--target_file", type=str,
                    help="target image file, located in <target_videos>")
...
target = f"{abs_path}/target_videos/{args.target_file}"
...
output_dir = f"{abs_path}/output_sketches/{test_name}/frame/"
...
def run(seed, wandb_name):
    exit_code = sp.run(["python", "-O", "painterly_rendering.py", target,
                            "--name", str(os.path.splitext(args.target_file)[0]),
                            ...
                            "--frame_cut", str(int(args.frame_cut)),
                            "--modify", str(int(args.modify))])
...
for seed in seeds:
    wandb_name = f"frame_{args.num_strokes}strokes_seed{seed}"
    if multiprocess:
...
if args.display:
    time.sleep(10)
    P.apply_async(display_, (0, f"frame_{args.num_strokes}strokes_seed0"))
