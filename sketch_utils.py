...
def get_mask_u2net(args, pil_im):
    ...
    predict = pred
    eps = 0.5
    predict[predict < eps] = 0
    predict[predict >= eps] = 1
    ...
    return im_final, predict, mask

def get_mask_inspyrenet(args, pil_im, fps):
    w, h = pil_im.size[0], pil_im.size[1]
    mask_video_path = args.target.replace("target_images", "InSPyReNet/results")
    video = cv2.VideoCapture(mask_video_path)
    video.set(cv2.CAP_PROP_POS_FRAMES, fps)
    ret, frame = video.read()
    video.release()
    frame = torch.tensor(frame).to(torch.float)
    gray_image = frame.permute(2, 0, 1)
    gray_image = gray_image.mean(0, keepdim=True)
    pred = gray_image
