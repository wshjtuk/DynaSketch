...
class Painter(torch.nn.Module):
    def __init__(self, args,
        ...
        self.rotation_theta = nn.Parameter(torch.tensor(0., requires_grad=True))
        self.matrix = torch.zeros((self.num_paths, self.num_paths), requires_grad=True)
    ...
    def get_path(self):
        ...
        p0 = self.inds_normalised[self.strokes_counter] if self.attention_init else (
    ...
    def render_warp(self):
        ...
        # for path in self.shapes:
        #     print("change color")
        #     img[path.points.data[0]] = torch.tensor([122., 233., 0.])
        #     img[path.points.data[3]] = torch.tensor([0., 122., 233.])
        return img
    ...
    def parameters(self):
        ...
        self.points_vars.append(self.rotation_theta)
    ...
    def dino_attn(self):
        ...
        dino_model = torch.hub.load(
            'facebookresearch/dino:main', 'dino_vits8').eval().to(self.device)

        # self.main_im = Image.open(self.target_path).convert("RGB")
        self.main_im = Image.fromarray(self.target_im.cpu().squeeze(0).
                                       permute(1, 2, 0).mul(255).
                                       byte().numpy()).convert("RGB")
        ...
        th_attn = nn.functional.interpolate(th_attn.unsqueeze(
            0), scale_factor=patch_size, mode="nearest")[0].cpu()

        attn = attn.reshape(nh, w_featmap, h_featmap).float()
        attn = nn.functional.interpolate(attn.unsqueeze(
            0), scale_factor=patch_size, mode="nearest")[0].cpu()
    ...
    def clip_attn(self):
        ...
        if "RN" in self.saliency_clip_model:
            ...
            attn_map = interpret(self.image_input_attn_clip,
                                 text_input, model, device=self.device)
    ...
    def set_inds_clip(self):
        ...
        self.attention_map_soft = attn_map_soft
        ...
        # if self.args.track:
        #     self.inds_normalised = np.load("inds/inds_result.npy")
        #     self.inds[:, 1] = self.inds_normalised[:, 0] * self.canvas_width
        #     self.inds[:, 0] = self.inds_normalised[:, 1] * self.canvas_height
        #     self.inds_normalised = self.inds_normalised.tolist()
        #     # self.inds_normalised[:, 0] =  (self.inds_normalised[:, 0] * self.canvas_width).astype(np.int) / self.canvas_width
        #     # self.inds_normalised[:, 1] =  (self.inds_normalised[:, 1] * self.canvas_height).astype(np.int) / self.canvas_width
        #     print("=====loading npy.=====")
        # else:
        #     np.save("inds/inds.npy", self.inds_normalised)
        #     print("=====saving npy.=====")
        # print(self.inds_normalised)
    ...
    def set_inds_dino(self):
        ...
        for i in range(num_heads):
            ...
            # all heads.
            topk, indices = np.unique(
                self.attention_map[i].numpy(), return_index=True)
            topk = topk[::-1][:k]
        ...
        prob_sum = sum_attn[self.inds[:, 0].astype(
            np.int), self.inds[:, 1].astype(np.int)]
        prob_sum = prob_sum / prob_sum.sum()
        new_inds = []
        for i in range(self.num_stages):
            new_inds.extend(np.random.choice(
                range(self.inds.shape[0]), size=self.num_paths, replace=False, p=prob_sum))
    ...
    def get_curve(self, point_num):
        ...
        new_cp_2 = []
        for path in shapes:
            new_start_point.append(path.points[0].to(self.args.device))
            new_cp_1.append(path.points[1].to(self.args.device))
            new_cp_2.append(path.points[2].to(self.args.device))
            new_end_point.append(path.points[3].to(self.args.device))
        new_bezier = []
        for bezier in range(len(new_start_point)):
            shape1 = []
            t = np.linspace(0, 1, num=point_num)
            for t_ in t:
                b1 = (1-t_)**3*new_start_point[bezier] + 3*(1-t_)**2*t_*new_cp_1[bezier] + 3*(
                    1-t_)*t_**2*new_cp_2[bezier] + t_**3*new_end_point[bezier]
                shape1.append(b1.to(self.args.device))
            new_bezier.append(shape1)
        return new_bezier
            
    def grouping(self, threshold):
        curve_points = self.get_curve(100)
        num_curves = len(curve_points)
        matrix = torch.zeros((num_curves, num_curves), dtype=torch.float).to(self.device)
        for i in range(num_curves):
            for j in range(i + 1, num_curves):
                curve1_tensor = torch.stack(curve_points[i])
                curve2_tensor = torch.stack(curve_points[j])
                distances = torch.cdist(curve1_tensor, curve2_tensor)
    ...
    def delImage(self):
        ...
        for i in range(len(self.shapes)):
            ...
            temp_self.shapes.pop(i)
            temp_self.shape_groups.pop(i)
            temp_selfs.append(temp_self)
        return temp_selfs
...
def interpret(image, texts, model, device):
    ...
    image_relevance = image_relevance.reshape(
        224, 224).data.cpu().numpy().astype(np.float32)
